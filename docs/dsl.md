...
Where a declaration is *itself* proof the value cannot work, Weft says so before serving rather than waiting for the render that trips over it. Three cases are refused with `Weft::InvalidDefinition`: naming a key no door declares, the pins-only identity above, and a declared `default:` or pinned value that is not a scalar, since a non-scalar fallback is proof the key can hold one. These are checked across the registered set, not in the macro, because `identifies_by` may legitimately appear above the `param` lines it names. They ride the same pass as [route and id-base collision checks](routing.md#collision-detection), and `Weft.registry.validate!` runs the lot on demand, so an app can fail at boot or answer a health check rather than waiting for its first request.

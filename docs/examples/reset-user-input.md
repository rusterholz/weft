...
**Clearing the params in the return keeps the re-render honest.** The callable ends with `{ author: nil, body: nil }`, and a returned hash merges into the params for the re-render ([the callable contract](../dsl.md#the-callable-contract)). Left populated, the submitted values would still be sitting in `params` while `build` ran, so anything reading them would echo the text the user just sent. The wrapper's id is unaffected either way: this component declares no `identifies_by`, so it comes back as `#comment-section` on every render, which is what the htmx wiring points at.

...
**A successful dismissal responds with nothing.** htmx removes the row on its own, so the response carries no body — Weft doesn't render the component it just helped delete, which is why `build` can assume its record exists without guarding. (Out-of-band [`brings`](../dsl.md#brings--companions-in-the-same-response) companions still ride the response when declared — a delete can update the count beside the table it shrinks.) The trailing `nil` in the callable keeps `Hash#delete`'s return value — the deleted record — from merging into the params those companions read. If the callable *raises*, Weft overrides the destructive swap (via `HX-Reswap`) so the error rendering appears where the row was, instead of the row silently vanishing — and the error fragment arrives as a `<tr>` itself, because recovery fragments [adopt the failing component's tag](../error-handling.md#auto-injected-recovery-params), landing as valid table content.

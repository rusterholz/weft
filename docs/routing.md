...
**You can ask for the pass rather than wait for it.** `Weft.registry.validate!` runs every check here plus the [identity declaration checks](dsl.md#identity), so a broken declaration can fail at boot instead of on a visitor's first page, and a health endpoint can answer whether the process is able to serve. It stays cheap to call repeatedly: a pass that succeeded is memoized, and one that found a problem is not, so a process that is still broken keeps reporting it.

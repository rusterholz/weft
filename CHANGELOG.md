...
- **`derives(:name) { |params| ... }` — lazy server-side derivations.** The third door into `params`, replacing the find-by-id dance at the top of every `build`. Declaring a derivation registers it; the block runs (at most once per render) when `params.name` is first read — and never runs if nothing reads it. Derivations chain lazily: a block that reads `params.order` forces the `order` derivation on demand, so `derives(:order) {...}` plus `derives(:shipments) { |p| Shipment.for_order(p.order.id) }` computes exactly what the render touches. Derived values are server-side: never serialized into URLs, not routable-making — but they do flow down the render tree like everything else in the bag, and a value an ancestor already computed is *not* recomputed by children.
  - **Blocks are `(params) -> value` pure functions.** They execute against a sandboxed `self` — `params` and lexical constants are in reach and `Kernel` stays available (`raise`, `format`, `Integer()`), but nothing component-specific is, so a bare method call raises `NameError`. Each block runs in its own fresh, disposable context: scratch instance variables are allowed but never outlive that one execution. A failing derivation raises at first read — inside the `recovers`-wrapped render, where `recovers from: ActiveRecord::RecordNotFound` and friends already do the right thing. A derivation nobody reads never raises.

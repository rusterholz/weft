...
RSpec.describe Weft::DSL::Params do
  ...
  describe ".param" do
    ...
    it "reserves no name for weft — a ticket is not a param, so it claims no key" do
      # The ticket belongs to the component instance, never to the params bag,
      # which is what leaves the whole namespace to the user.
      klass = Class.new(base_class) do
        def self.name = "TicketNamedAttr"
        param :_ticket, default: "mine"
      end

      expect(klass.params[:_ticket]).to eq(default: "mine")
    end
